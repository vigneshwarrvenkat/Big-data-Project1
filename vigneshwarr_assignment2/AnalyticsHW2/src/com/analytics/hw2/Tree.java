...
public class Tree {
	public Node buildTree(ArrayList<Record> records, Node root, LearningSet learningSet) {
		...
		for(int i = 0; i < Hw2.NUM_ATTRS - 2; i++) {
			if(!Hw2.isAttributeUsed(i)) {
				double entropy = 0;
				ArrayList<Double> entropies = new ArrayList<Double>();
				ArrayList<Integer> setSizes = new ArrayList<Integer>();
				
				for(int j = 0; j < Hw2.NUM_ATTRS - 2; j++) {
				...
				System.out.println("Entropy" + gain);
		...
		if(bestAttribute != -1) {
			int setSize = Hw2.setSize(Hw2.attrMap.get(bestAttribute));
			root.setTestAttribute(new Features(Hw2.attrMap.get(bestAttribute), 0));
			root.children = new Node[2];
			root.setUsed(true);
			Hw2.usedAttributes.add(bestAttribute);
			
			for (int j = 0; j< setSize; j++) {
				root.children[j] = new Node();
				root.children[j].setParent(root);
				root.children[j].setData(subset(root, bestAttribute, j));
				root.children[j].getTestAttribute().setName(Hw2.getLeafNames(bestAttribute, j));
				root.children[j].getTestAttribute().setValue((float) j);
			}
